...
public class EcommerceSystemService {
    ...
    public HashMap<String, Category> getCategories() {return system.getCategories();}
     ...
     */
    public void sendOutOfStockMsgToSeller(Product product){
        String sellerId = product.getSellerId();
        User sellerUser = findUserById(sellerId);
        if (sellerUser instanceof Seller seller) {
            String notificationContent = "Product ID: " + product.getProductId() + " - " + product.getProductName() + " is out of stock.";
            seller.getNotifications().add(new Message(Message.MessageType.OUT_OF_STOCK, notificationContent));
            System.out.println("Notification sent to seller about out-of-stock product: " + product.getProductId());
        }
    }
